 ...
 *
 * The Explorer already ships its own toggle (`button.title-button.explorer-toggle` around an <h2>),
 * so it's left alone. Recent Notes and Tag Explorer only have a plain <h3>, so we wrap each in an
 * equivalent toggle button and remember the open/closed state in localStorage — matching the
 * Explorer's own `data-savestate` behaviour.
 */
...
function makeCollapsible(panel: HTMLElement, key: string, defaultOpen: boolean) {
