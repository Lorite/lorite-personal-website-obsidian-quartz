 ...
 *
 * `appendToggle`: when true, the toggle is APPENDED (last child) rather than prepended, and shown
 * first via CSS `order: -1`. This is required for the Explorer: it renders its file tree client-side
 * by cloning `<template>` elements that live as the last children of `.explorer`, and Quartz's
 * micromorph reconciles the DOM by child position on SPA navigation. A prepended button shifts every
 * following child by one, so on the next navigation micromorph mis-matches the templates and strips
 * the folder template's inner container — the tree then collapses to just its top-level folders
 * (looks empty). Appending keeps the templates in their original positions.
 */
...
function setup() {
  ...
  if (explorer instanceof HTMLElement) {
    ...
    const section = buildSection(
      ...
      true, // append the toggle — see buildSection (micromorph corrupts the Explorer's templates otherwise)
    )
    if (section) sections.push(section)
  }
